...
class CIFAR100(Dataset):
    ...
    def __init__(
        ...
        for file_name, checksum in downloaded_list:
            ...
            with open(file_path, "rb") as f:
                ...
                key_data = b"data" if b"data" in entry else "data"
                key_labels = b"labels" if b"labels" in entry else "labels"
                key_fine_labels = b"fine_labels" if b"fine_labels" in entry else "fine_labels"

                self.data.append(entry[key_data])
                if key_labels in entry:
                    self.targets.extend(entry[key_labels])
                else:
                    self.targets.extend(entry[key_fine_labels])
    ...
    def _load_meta(self) -> None:
        ...
        self.classes.append("adversarial")
